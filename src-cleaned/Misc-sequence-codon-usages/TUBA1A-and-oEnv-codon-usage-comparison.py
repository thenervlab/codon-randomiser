...
#---------------------------------------Repeat for oEnv protein codon usage----------------------------------------
...
protein_proportions_original_oENV = []
for codons in codons_by_protein:
    # Get the protein name from the group
    protein_name = codons['protein'].iloc[0]
    # Count codons per amino acid and codon
    ...
    proportion['optimal_freq-original'] = proportion['optimal_frequency'] - proportion['proportion']
    proportion['protein'] = protein_name  # Add protein column
    protein_proportions_original_oENV.append(proportion)

protein_proportions_original_oENV = pd.concat(protein_proportions_original_oENV)
protein_proportions_original_oENV['sequence'] = 'oENV'

#------------------------------Combine dataframes and save----------------------------
# Concat both tables
concat_sequences = pd.concat([protein_proportions_original_oENV, protein_proportions_TUB], ignore_index=True)
