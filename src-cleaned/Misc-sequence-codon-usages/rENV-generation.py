...
# Calculate codon identity with original sequence
...
# -------------------------------------Identify TG repeats in original codon--------------------------------
...
#-----------------------------------Rabdomise codons in RRRR repeats-----------------------------------
stringency = 'high'
new_codons = []
...
codons['pre-syn_codon'] = randomise_codons(codons)

# Re-calculate codon identity

nt_identities = []
for old, new in codons[['original_codon', 'pre-syn_codon']].values:
    if old == new:
        nt_identities.append(3)
    else:
        nt_identity = 0
...
codons['aminoacid'] = codons['pre-syn_codon'].map(amino_dic)
...
for codons in codons_by_protein:
    ...
    proportion = (
        codons[['aminoacid', 'pre-syn_codon', 'final_nucleotide_identity']]
        .groupby(['aminoacid', 'pre-syn_codon'])
    ...
    proportion['optimal_frequency'] = proportion['pre-syn_codon'].map(codon_frac_dic)
...
new_codon_comparison = protein_proportion_new[['pre-syn_codon', 'aminoacid', 'rand_RSCU', 'protein','optimal_freq-new']].copy()

new_codon_comparison['sequence'] = 'rENV'

output_csv_path = os.path.join(output_folder, 'codon-usage-randomised_ENV-sequence.csv')
new_codon_comparison.to_csv(output_csv_path, index=False)
