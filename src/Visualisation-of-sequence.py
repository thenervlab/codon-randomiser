...
# codons = pd.read_csv('results/Optimised-sequences_ENV.csv')
...
# final_codons.sort_values(['aminoacid'])
...
GTG_repeats = codons[codons['final_codon'] == 'GTG'].copy()
TGT_repeats = codons[codons['final_codon'] == 'TGT'].copy()
...
codons['GTGT_clustered?']= [1 if x in flattened_clusters else 0 for x in codons.index.tolist()]
